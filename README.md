...
- **管理员物品默认隐藏**（基岩、屏障、结构方块、刷怪笼、龙蛋、测试方块等生存不可获取物品），可在管理界面一键放开
- **数据库支持 SQLite / MySQL**：分类开关、物品黑白名单、玩家各分类记忆页码全部入库，游戏内实时管理
- **管理员 GUI**（`/mats admin`）：黑名单列表 / 白名单列表 / 分类与功能开关，物品点击即在黑白名单间切换
- **获取背包已有的物品**：把想要的物品放进背包点一下即可筛出，替代搜索
- 取物冷却可配置（默认 1 秒），防刷屏
...
| 命令 | 说明 | 权限 |
|---|---|---|
| `/mats`（别名 `/materials`、`/cailiao`） | 打开材料菜单 | `architectblocks.use`（拥有 admin 权限也视为可使用） |
